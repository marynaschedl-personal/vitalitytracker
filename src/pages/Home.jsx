...
export default function Home() {
  ...
  async function loadData() {
    ...
    // Clear and reseed measurements to ensure correct data
    let allMeasurements = await base44.entities.Measurement.list("-date", 200);

    // Clear all existing measurements and reseed with correct data
    if (allMeasurements.length > 0) {
      await Promise.all(allMeasurements.map((m) => base44.entities.Measurement.delete(m.id)));
    }

    // Seed with correct measurements
          ...
          {showLogoutMenu && (
            <div className="absolute right-0 mt-2 bg-card border border-border rounded-lg shadow-lg z-50">
              <button
                ...
                className="w-full px-4 py-2 flex items-center gap-2 text-destructive hover:bg-destructive/10 rounded-lg text-sm"
              >
